...
sudo /System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport awdl0 -z

echo "Setting en0 MAC address to $MAC0"
sudo ifconfig en0 ether "$MAC0"
sudo ifconfig en0 lladdr "$MAC0"

echo "Setting awdl0 MAC address to $MAC1"
sudo ifconfig awdl0 ether "$MAC1"
sudo ifconfig awdl0 lladdr "$MAC1"
